...
def get_lm_pipeline(model: PreTrainedModel):
    ...
    if model_class == "LlamaForCausalLM":
        ...
        model_class == "ParamForCausalLM" or model_class == "ParamBharatGenForCausalLM"
    ):
        return nn.Sequential(model.transformer.ln_f, model.lm_head)
...
def add_tsv_layers(model: PreTrainedModel, tsv: Tensor, alpha: list, args: Args):
    layers = get_layers(model)
    mlp_keywords = ["mlp", "feedforward", "ffn"]
        ...
        for i, layer in enumerate(layers):
            if i == args.str_layer:
                ...
                layers[i] = LlamaDecoderLayerWrapper(
                    decoder_layer, TSVLayer(tsv[i], alpha), args.model_name
                )
