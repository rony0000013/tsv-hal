...
def get_last_non_padded_token_rep(hidden_states, attention_mask):
    ...
    last_token_reps = torch.stack(
        [hidden_states[i, lengths[i] - 1, :] for i in range(batch_size)]
    )
...
def compute_entropy(last_token_rep, centroids, pseudo_label, k, cls_dist, args):
    ...
    if (
        len(ce_class_0) < cls0_num or len(ce_class_1) < cls1_num
    ):  # Fallback to top-k across all classes
        _, top_k_indices = torch.topk(ce, k, largest=False, sorted=True)

    else:
        top_0_indices = cls_0_indices[
            torch.topk(ce_class_0, int(cls0_num), largest=False, sorted=True).indices
        ]
        top_1_indices = cls_1_indices[
            torch.topk(ce_class_1, int(cls1_num), largest=False, sorted=True).indices
        ]
